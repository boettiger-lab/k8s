...
# vllm/nimbus/up.sh, run last, once the GPU is actually being advertised.
...
helm upgrade -i dcgm-exporter gpu-helm-charts/dcgm-exporter \
  ...
  --values "$REPO_ROOT/monitoring/dcgm-exporter-values.yaml"
...
for s in huggingface-token vllm-api-key ngc-pull; do
    ...
    if [ -f "$f" ]; then
        ...
        echo "    MISSING $f -- recreate $s by hand before vllm/nimbus/up.sh" >&2
    fi
done
...
kubectl apply -f "$REPO_ROOT/mcp/nimbus/"
echo
echo "    vLLM is deliberately left to you -- it needs NIMBUS_KEY in the env:"
echo "      cd $REPO_ROOT/vllm/nimbus && NIMBUS_KEY=... ./up.sh"
