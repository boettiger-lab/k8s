...
|---|---|
| `mcp-gpu-nimbus` Deployment/Service/Ingress | committed to `services/mcp/` — it had never been in git |
| `nimbus-carbon-api` | archived to `platform/monitoring/nimbus-carbon-api.yaml`, deliberately **not** redeployed (see below) |
| `ngc-pull`, `huggingface-token`, `vllm-api-key`, `ghcr-pull` | `01-backup-nimbus.sh` dumps them; `03-post-join.sh` restores them |
| the vLLM endpoint | `services/vllm/endpoints.yaml` + `up.sh`, now in namespace `vllm` |
...
`../`, and the `openebs-zpool` ZFS pool.
...
cluster. So does `platform/nvidia/nvidia-device-plugin.sh`. Once step 3 is done, replace
...
unset KUBECONFIG
cd ~/Documents/github/boettiger-lab/k8s/cluster/nodes/nimbus/join
...
runs `k3s-killall.sh` first for this reason. See `../README.md`.

**Two k3s configs now.** `../` carries both
