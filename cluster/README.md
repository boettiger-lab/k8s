# cluster — the machines

[k3s](https://docs.k3s.io/installation) is easily the best way to provide a self-hosted
k8s environment. Lightweight, batteries-included; the default deployment works well, with
a few additional steps to configure nvidia and friends.

**One cluster, three nodes:** cirrus is the server (control plane); thelio and nimbus are
agents. This directory covers installing and joining nodes, remote kubectl access,
automated k3s upgrades ([`upgrade/`](upgrade/)), and per-node host configuration
([`nodes/`](nodes/) — kernel tunables, GPU watchdog, lockup capture).

Everything that runs *on* the cluster is in [`../platform/`](../platform/) (the supporting
layer) and [`../services/`](../services/) (what users get).
...
See [`../platform/nvidia/`](../platform/nvidia/) for more details on GPU configuration.
...
| [`nodes/thelio/`](nodes/thelio/) | **thelio** (amd64, consumer board, non-ECC, RTX 2080) | freeze *detection* — `hung_task_panic` and full SysRq, so a wedge reboots itself and can be diagnosed from the console |
| [`nodes/nimbus/`](nodes/nimbus/) | **nimbus** (DGX Spark GB10, unified memory) | a runaway GPU pod exhausting the *shared* RAM pool — vLLM budgeting, swap sizing, and the GPU hang watchdog |
...
- **thelio's JuiceFS wedge** — a *runtime* filesystem hang. A stalled FUSE
  ...
  [`../platform/juicefs/README.md`](../platform/juicefs/README.md) gotcha 3.
