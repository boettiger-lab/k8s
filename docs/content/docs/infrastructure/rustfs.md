...
[JuiceFS]({{< relref "shared-home-storage" >}}) — the `juicefs-homes` bucket, which
holds user home directories — and a general S3 target for large datasets.

RustFS is documented here as **infrastructure, not a user-facing service**: it exists so
JuiceFS can offer ReadWriteMany homes. Object storage you address directly is
[MinIO]({{< relref "../services/minio" >}}); the RustFS console is an admin interface.
...
> ([`cluster/nodes/nimbus/join/`](https://github.com/boettiger-lab/k8s/tree/main/cluster/nodes/nimbus/join)).
> Its PVC held 276 KiB and nothing was migrated. Everything below is cirrus.

## Deployment

One manifest, `platform/rustfs/cirrus.yaml`, holding the Namespace, PVC, Deployment,
...
```bash
kubectl apply -f platform/rustfs/cirrus.yaml
```
