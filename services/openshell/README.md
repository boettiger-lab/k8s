...
plan and needs no cluster changes. On cirrus it is the `openshell` **`.deb`** (no apt
repo), with `openshell-gateway` as a systemd **`--user`** unit registered at
`https://127.0.0.1:17670`; re-running `install.sh` as `cboettig` upgrades the package
and restarts the unit. Installed 0.0.110-1 as of 2026-09-07.

Upstream version state re-checked 2026-09-07: OpenShell **v0.0.116** (2026-08-28),
agent-sandbox **v1.0.1**.
...
Also note the OpenShell Helm chart is still explicitly labelled **Experimental**: *"the
Kubernetes deployment path is under active development. Expect rough edges and breaking
changes."* (wording as of 2026-09-07; slightly softer than the earlier "Do not use it in
production", but the label itself has not changed, and chart versions are still `0.0.x`).
Releases land every day or two — 0.0.109 → 0.0.116 in nine days. That churn is survivable
for a course environment on a disposable worker; it is not survivable on cirrus.
...
Agent Sandbox has since crossed **1.0** (v1.0.1, controller image
`registry.k8s.io/agent-sandbox/agent-sandbox-controller:v1.0.1`) — the riskiest
cluster-scoped dependency in this plan is the most settled part of it. The CRD is
`Namespaced` and serves **only `v1beta1`** now; the chart accepts `v1beta1` or
`v1alpha1`, so this is fine.

Things to remember:

- **The asset was renamed.** It is `sandbox.yaml`, not `manifest.yaml`, as of v1.0.1 —
  and `releases/latest/download/manifest.yaml`, which the OpenShell chart README still
  tells you to apply, now 404s. `sandbox.yaml` is the CRD + controller. The release also
  ships `extensions.yaml` (`SandboxClaim`, `SandboxTemplate`, `SandboxWarmPool`) and a
  combined `sandbox-with-extensions.yaml`; OpenShell does not need the extensions, so
  apply `sandbox.yaml` only.
- **Pin the release.** An unpinned apply means an unreviewed CRD change lands whenever we
  re-run it. Vendor the manifest into this directory when we deploy.
...
`guardrails.yaml`. The chart creates **no** ResourceQuota or LimitRange, and its
...
helm upgrade --install openshell oci://ghcr.io/nvidia/openshell/helm-chart \
  --version 0.0.116 -n openshell -f values.yaml
kubectl -n openshell rollout status statefulset/openshell
```

Pin the chart version (releases are `0.0.x`; `0.0.116` as of 2026-09-07) and keep the
**CLI and gateway versions in lockstep** — they are released together. Do not use the
floating `0.0.0-dev` / `0.0.0-dev.<sha>` chart tags; they track `main` and are overwritten.

The gateway workload kind is now selectable: `workload.kind` defaults to `statefulset`
(SQLite), so `rollout status statefulset/openshell` above is still right. `deployment` is
for external-PostgreSQL multi-replica installs, which we do not want.

### Chart changes since this plan was written

Re-checked against chart 0.0.116 on 2026-09-07:

- **Preflight gate.** The chart now fails before creating gateway resources if the
  cluster serves neither Sandbox API. Good — it makes phase 1 self-enforcing. Pass
  `agentSandbox.preflight.enabled=false` only for offline `helm template` linting.
- **`server.appArmorProfile` defaults to `Unconfined`** for sandbox agent containers,
  because `runtime/default` blocks the supervisor's network-namespace mount setup. Worth
  knowing that this is a deliberately relaxed default we inherit, and another reason the
  sandbox node should hold nothing.
- **`server.drivers.kubernetes.workspaceMode` gained `operator`** — the gateway uses
  *pre-provisioned* namespaces (discovered by `operatorNamespaceLabel` or listed in
  `operatorNamespaceFile`) instead of auto-creating them. This looks better for a course
  than `managed`: we create one namespace per course with our own ResourceQuota,
  LimitRange, and egress NetworkPolicy already in place, rather than hoping guardrails
  reach a namespace the gateway made. Revisit the phase-3 choice when we deploy.
- **`server.workspaceDefaultStorageSize`** (default 2Gi) sets the sandbox workspace PVC
  size; set it alongside `workspaceStorageClass: openebs-zfs`.
- **`server.policyValidationFailureMode`** defaults to `fail_closed` (a candidate policy
  that fails validation deactivates the previous one). Correct default for us.
- **Still gateway-only:** top-level `nodeSelector` / `tolerations` / `affinity` apply to
  the gateway pod. Confirmed in 0.0.116 `values.yaml` — the sandbox-placement gap in
  phase 3 stands.
- **Unreleased on `main`:** a gateway/workspace split — `workspaceResources.enabled=false`
  plus a separate `openshell-workspace` chart installed per workspace namespace, so the
  two can be upgraded independently. Not in 0.0.116; check whether it has landed before
  committing to a workspace layout.
