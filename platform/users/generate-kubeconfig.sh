...
#   SERVER_ADDRESS="<server-ip-or-dns-only-host>" ./generate-kubeconfig.sh [USERID]
# or
#   K3S_SERVER_ADDRESS="<server-ip-or-dns-only-host>" ./generate-kubeconfig.sh [USERID]
#
# Notes:
...
if [[ -n "${SERVER_OVERRIDE}" ]]; then
  ...
  echo "  sudo ./generate-kubeconfig.sh ${USERID} --server <server-ip-or-dns-only-host>"
fi
